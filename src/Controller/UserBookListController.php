...
class UserBookListController extends AbstractController
{
    ...
    public function index(Request $request, BookRepository $bookRepository): Response
    {
        ...
        if ($searchForm->isSubmitted() && $searchForm->isValid()) {
            ...
            $paginator = $bookRepository->getUserSearchPaginator($search, $user->getUserIdentifier(), $currentPage);
        }

        return $this->render('user_book_list/index.html.twig', [
            'search_form' => $searchForm,
            'paginator' => $paginator ?? $bookRepository->getUserPaginator($user->getUserIdentifier(), $currentPage),
        ]);
    }
}
